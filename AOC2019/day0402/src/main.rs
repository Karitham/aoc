...
fn main() {
    ...
    for current_number in range.min..range.max {
        ...
        if b.is_sorted() && same_n_appears_twice(b) {
...
fn same_n_appears_twice(numbers: &[u8]) -> bool {
    let mut set: HashMap<u8, bool> = HashMap::new();

    for i in 0..numbers.len() - 1 {
        if numbers[i] == numbers[i + 1] {
            if set.contains_key(&numbers[i]) {
                set.insert(numbers[i], false);
                continue;
            }
            set.insert(numbers[i], true);
        }
    }
